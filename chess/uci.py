...
class Engine(object):
    ...
    def go(self, searchmoves=None, ponder=False, wtime=None, btime=None, winc=None, binc=None, movestogo=None, depth=None, nodes=None, mate=None, movetime=None, infinite=False, async_callback=None):
        ...
        *depth*, *nodes*, *mate*, *infinite* or some time control settings,
        ...
        if self.chess_db:
            db_ret = chessdb.get_pv(self.chess_db_pos)
            ponderMove = None
            bestmove = db_ret["pv"][0]
            if len(db_ret["pv"]) > 1:
                ponderMove = db_ret["pv"][1]
            result = bestmove, ponderMove
            # print("DB:", result)
            self._queue_command(GoCommand(searchmoves, ponder, wtime, btime, winc, binc, movestogo, 1, nodes, mate, movetime, infinite), async_callback)
        else:
            result = self._queue_command(GoCommand(searchmoves, ponder, wtime, btime, winc, binc, movestogo, depth, nodes, mate, movetime, infinite), async_callback)
            # print("NODB:", result)
        return result
